# ============================================================
# Script 5: Analysis (Node ±window) + Macro breakdown + Permutation
# INPUT:
#   exports/<tag>/<tag>_alignment.xlsx
# OUTPUT:
#   results/<tag>/  (files prefixed by <tag>_)
...
# ----------------------------
# 0) ONLY EDIT THIS (PUBLIC TEMPLATE)
# ----------------------------
# IMPORTANT:
# - Do NOT commit personal local paths (D:/..., C:/Users/...).
...
# ---- analysis parameters (safe to commit) ----
window_s <- 2

# ---- permutation settings (safe to commit) ----
B_perm    <- 2000
seed_perm <- 42
...
# ----------------------------
# Expected structure under project root:
#   exports/<tag>/<tag>_alignment.xlsx
#   results/<tag>/

proj_root <- get_env_or(
  "GESTURE_ROOT",
...
point_in_intervals <- function(t_vec, intervals) {
  ...
  for (i in seq_along(t_vec)) {
    t <- t_vec[i]
    while (j <= nrow(intervals) && t > intervals$end[j]) j <- j + 1L
    if (j <= nrow(intervals) && t >= intervals$start[j] && t <= intervals$end[j]) res[i] <- TRUE
  }
  res
}
...
# CHECK INPUT
# ----------------------------
if (!file.exists(xlsx_path)) stop("Missing: ", xlsx_path)
...
# Accept common column name variants for start/end
events <- events %>%
...
events <- events %>%
  mutate(
    ...
    event_id  = if ("event_id" %in% names(.)) event_id else row_number(),
    t_peak    = (start_sec + end_sec) / 2
  )
...
# ONLY use: time_s + Macro (no filters)
# ----------------------------
sp <- read_excel(xlsx_path, sheet = "StructuralPoints") %>% as_tibble()

if (!("time_s" %in% names(sp))) stop("StructuralPoints must contain column: time_s")

sp2 <- sp %>%
  mutate(
    time_s = as.numeric(time_s),
    Macro  = if ("Macro" %in% names(.)) as.character(Macro) else NA_character_,
...
sp_windows <- sp2 %>%
  transmute(
    PointID = if ("PointID" %in% names(sp2)) as.character(.data$PointID) else as.character(row_number()),
    Macro,
...
compute_lift_from_points <- function(events_t, point_times, T_total, window_s) {
  if (length(point_times) == 0) {
    return(tibble(Lift=NA_real_, r_in=NA_real_, r_out=NA_real_,
  ...
  for (i in seq_along(events_t)) {
    t <- events_t[i]
    while (j <= nrow(union) && t > union$end[j]) j <- j + 1L
    if (j <= nrow(union) && t >= union$start[j] && t <= union$end[j]) in_win[i] <- TRUE
  }
...
perm_test_shift <- function(events_t, point_times, T_total, window_s,
  ...
  obs <- compute_lift_from_points(events_t, point_times, T_total, window_s)$Lift
